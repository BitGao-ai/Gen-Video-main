...
ARGS=(
  ...
  --steps 20 --num-frames 49 --height 384 --width 640 --vae-tile 128
  --quality "$QUALITY" --seed "$SEED" --device cuda
)
if [[ "$DRY_RUN" == 1 ]]; then
  printf 'CUDA_VISIBLE_DEVICES=%q ' "$GPU_IDS"
  printf '%q ' "$PYTHON_BIN" -u "${ARGS[@]}" --output "$OUTPUT_ROOT/<unique-run>/accelerated.mp4"
...
ARGS+=(--output "$run_dir/accelerated.mp4")
echo "[start] GPU=$GPU_IDS quality=$QUALITY seed=$SEED"
